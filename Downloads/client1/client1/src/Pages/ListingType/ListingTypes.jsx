...
const ListingTypePage = () => {
  ...
  const addLysting = () => {
    navigate("/admin/add-listing-types");
  };

  const handleEdit = () => {
    navigate(`/admin/edit-listing-types/${editId._id}`,{
  ...
  const getData = async () => {
    try {
      let data = await LystingTypeService.getListingType(); 
      setRow(data?.data || []);
    } catch (error) {
  ...
  const handleDelete1 = async () => {
    try {
      await LystingTypeService.deleteListingType({}, deleteId);
      getData();
  ...
  return (
    ...
    />
      <div className="text-start mb-2 mt-5 ms-1" style={{ fontWeight: "800" }}>
        <Link to="/admin/dashboard">Dashboard</Link>&nbsp;&#8811;&nbsp; Listing Types
      </div>
      <Row>
        <Col xl={12}>
          <Card>
            <Card.Header className="d-flex justify-content-between">
              <Card.Title className="fw-bold fs-5 mt-2">Listing Type</Card.Title>
