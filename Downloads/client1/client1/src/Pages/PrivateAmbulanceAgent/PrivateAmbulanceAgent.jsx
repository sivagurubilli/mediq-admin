...
const PrivateAmbulanceAgent = () => {
  ...
  const handleAdd = () => {
    navigate("/admin/add-private-abmulance-agent");
  };

  const handleEdit = () => {
    navigate(`/admin/edit-private-abmulance-agent/${editId?._id}`,{
  ...
  const getData = async () => {
    try {
      let data = await PrivateAmbulanceAgentService.getPrivateAmbulanceAgent();
  ...
  const handleDelete1 = async () => {
    try {
      let data = await PrivateAmbulanceAgentService.deletePrivateAmbulanceAgent({}, deleteId);
      getData();
  ...
  const filteredData = useMemo(() => {
    return row?.filter((item) => {
      return (
        item.privateAmbulanceagentName.toLowerCase().includes(searchInput.toLowerCase()) 
      );
    });
  ...
  return (
    ...
    <div className="text-start mb-2 mt-5 ms-1" style={{ fontWeight: "800" }}>
      <Link to="/admin/dashboard">Dashboard</Link>&nbsp;&#8811;&nbsp; Private Ambulance Agent
      ...
      <Col xl={12}>
        <Card>
          <Card.Header className="d-flex justify-content-between">
            <Card.Title className="fw-bold fs-5 mt-2"> Private Ambulance Agent</Card.Title>
...
export default PrivateAmbulanceAgent;
