...
import AmbulanceAdminData from "./AmbulanceAdminData"
...
const Sidebar = (props) => {
  ...
  const privateambulanceData = AmbulanceAdminData();
const hospitalAdminData = HospitalAdminData()
  const sidebarData = sidebarData1()
  const ref = useRef();
        ...
        <SimpleBar className="h-100" ref={ref}>
          <div id="sidebar-menu">
            <ul className="metismenu list-unstyled" id="side-menu-item">
              {(!userData?.isSuperadmin ? sidebarData : userData?.isPrivateAmbulance ?  privateambulanceData : hospitalAdminData  || []).map((item, key) => (
                <React.Fragment key={key}>
                  {item.isMainMenu ? (
                    <li className="menu-title">{props.t(item.label)}</li>
                  ) : (
                    <li key={key} onClick={item.onClick}
