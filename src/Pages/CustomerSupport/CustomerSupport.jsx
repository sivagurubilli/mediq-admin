...
const CustomerSupport = () => {
  ...
  const handleAdd = () => {
    navigate("/admin/add-customer-support");
  };

  const handleEdit = () => {
    navigate(`/admin/edit-customer-support/${editId?._id}`,{
  ...
  const getData = async () => {
    try {
      let data = await CustomerSupportService.getCustomerSupport();
  ...
  const handleDelete1 = async () => {
    try {
      let data = await CustomerSupportService.deleteCustomerSupport({}, deleteId);
      getData();
  ...
  return (
   ...
   <EditModal
    ...
    <div className="text-start mb-2 mt-5 ms-1" style={{ fontWeight: "800" }}>
      <Link to="/admin/dashboard">Dashboard</Link>&nbsp;&#8811;&nbsp; Customer Support Agents
      ...
      <Col xl={12}>
        <Card>
          <Card.Header className="d-flex justify-content-between">
            <Card.Title className="fw-bold fs-5 mt-2"> Customer Support Agents</Card.Title>
