...
const FirstAID = () => {
  ...
  const addPage =()=>{
    navigate("/admin/add-first-aid")
  }
  
  
  const handleEdit = () => {
    navigate(`/admin/edit-first-aid/${editId._id}`, {
  ...
  const getData =async()=>{
    try {
      let data = await FirstAidService.getfirstAid();
   setRow(data?.data)     
  ...
  const handleDelete1 = async () => {
    try {
      let data = await FirstAidService.deletefirstAid({}, deleteId);
      getData();
  ...
  return (
    ...
    <div className="text-start mb-2 mt-5 ms-1" style={{ fontWeight: "800" }}>
      <Link to="/admin/dashboard">Dashboard</Link>&nbsp;&#8811;&nbsp;  First Aid 
    </div>
    <Row>
      <Col xl={12}>
        <Card>
          <Card.Header className="d-flex justify-content-between">
            <Card.Title className="fw-bold fs-5 mt-2">  First Aid </Card.Title>
