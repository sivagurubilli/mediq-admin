...
const Ambulance = () => {
  ...
  const getData = async () => {
    try {
      let data = await AmbulanceService.getAmbulances();
  ...
  const handleDelete1 = async () => {
    try {
      let data = await AmbulanceService.deleteAmbulances({}, deleteId);
      getData();
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "ambulancetype",
      },
      {
        Header: " Ambulance Vehicle Number ",
  ...
  const filteredData = useMemo(() => {
    return row?.filter((item) => {
      return (
        item?.vehiclenumber?.toLowerCase().includes(searchInput.toLowerCase()) 
      );
    });
